  ...
  exports.insertAdmin = async (req, res) => {
    ...
    try {
      ...
      if(admin.length === 0)
      {
        ...
        const sql = "INSERT INTO `admins` SET role=?, name=?, email=?, password=?, gender=?, image=?, status='?'";
        const results = await db.query(sql, [role, name, email, hashedPassword, gender, admin_image, status]);
    ...
    try {
      ...
      if(admin.length > 0)
      {
        ...
        if(result.length > 0)
        {
          ...
          const sql = 'UPDATE `admins` SET name=?, email=?, password=?, gender=?, image=? WHERE id=?';
          const edit_results = await db.query(sql, [name, email, hashedPassword, gender, admin_image, id]);
  ...
  exports.updateProfileAdmin = async (req, res) => {
    ...
    try {
      ...
      if(admin.length > 0)
      {
        ...
        // Update data into the admin table
        const sql = 'UPDATE `admins` SET name=?, email=?, gender=?, image=? WHERE id=?';
        const edit_results = await db.query(sql, [name, email, gender, admin_image, admin_id]);
