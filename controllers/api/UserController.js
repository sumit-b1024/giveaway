...
exports.apiCreateUser = async (req, res) => {
  ...
  try {
    ...
    if(results.length === 0)
    {
      ...
      const insert_sql = "INSERT INTO `users` SET first_name=?, last_name=?, email=?, password=?, phone=?, gender=?, about=?, profile_img=?,  status='?'";
      const result2 = await db.query(insert_sql, [first_name, last_name, email, hashedPassword, phone, gender, about, profile_img, status]);
      ...
      if (result2.insertId > 0) {
        ...
        res.status(200).send({status: false, result: "", errors: "Error: "+error });
      }
    }else{
  ...
  try {
    ...
    if(users.length > 0)
    {
      ...
      res.status(200).send({ 
        ...
        res.status(200).send({status: false, result: "", errors: "user not found" });
    }
  }catch (error) {
  ...
  try {
    ...
    if(users.length > 0)
    {
      ...
      res.status(200).send({ 
        ...
        res.status(200).send({status: false, result: "", errors: "Not found user with " + search_term });
    }
  }catch (error) {
    res.status(500).send({ status: false, result: "", errors:'Error fetching data:'+error });
  }
    ...
    if(user.length > 0)
    {
      ...
      if(user2.length > 0)
      {
        res.status(404).send({status: false, result: "", errors: "Email is already exists. please try another Email." });  
          ...
          const sql = 'UPDATE `users` SET first_name=?, last_name=?, email=?, password=?, phone=?, gender=?, about=?, profile_img=? WHERE id=?';
          const edit_results = await db.query(sql, [first_name, last_name, email, hashedPassword, phone, gender, about, profile_img, user_id]);
          ...
          if (edit_results.affectedRows > 0) {
            ...
            res.status(200).send({status: false, result: "", errors: "User record has not updated" });
          }
      }
    }else{
